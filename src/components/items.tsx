...
export const Section = ({children, title}: {title: string; children:ReactNode}) => {
    ...
    return(
        <View style={{}}>
            <Text style={{marginTop:30, paddingLeft:20, alignSelf: width > height ? 'flex-start' : 'auto',}} variant="headlineMedium">{title}</Text>
            {children}
        </View>
    );
};
...
const PlatformChip = ({platform}:{platform:ProjectPlatforms}) => {
    ...
    switch(platform){
        case 'Android':
            return <Chip style={[styles.chip, {backgroundColor:'rgba(0, 240, 64, 0.3)'}]} icon="robot">Android</Chip>;
    }
};

const AppCardTitle = ({title, platforms}:{title:string, platforms:ProjectPlatforms[]|null}) => {
        ...
        <View style={[{flex:1, margin:15,}]}>
            <Pressable >
                <Card mode="contained" onPress={() => null} style={[animatedStyle]}>
                    ...
                    <Card.Title title={<AppCardTitle title={info.name} platforms={info.platforms}/>} subtitle={info.platforms && info.platforms[0]} />
                    <Card.Content>
