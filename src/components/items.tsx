    ...
    return (
        <View style={[width > height ? {flex:1} : undefined, { margin: 15 }]}>
                <Card mode="elevated" onPress={() => null}>
                    {info.coverImg && <Card.Cover source={typeof info.coverImg === 'string' ? {uri:info.coverImg} : info.coverImg} resizeMode='contain' style={{backgroundColor:'transparent', padding: 5}} />}
                    <Card.Title
